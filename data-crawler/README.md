...
Sammelt Echtzeit-Parkplatzdaten (PLS – Parkleitsystem) von fünf Schweizer Städten (Luzern, Basel, St. Gallen, Zürich, Bern), speichert sie als JSON-Historie und in einer MariaDB-Datenbank, und stellt sie über eine kleine Flask-API bereit. Ergänzend gibt es ein Skript zum Nachladen von historischen Wetter- und Event-Daten.
...
| [`db_utils.py`](db_utils.py) | Python | DB-Hilfsfunktionen (`get_connection`, `insert_measurement`, `insert_log`) |
...
| [`web_server.py`](web_server.py) | Python | Flask-Webserver/API zum Anzeigen der gesammelten Daten |
| [`cities.json`](cities.json) | Config | Städte-Konfiguration (API-URLs, Collector-Zuordnung, enabled-Flag) |
...
| [`.env`](.env) | Config | DB-Zugangsdaten (nicht versioniert einsehen – enthält Secrets) |
| `data/` | Ordner | Abgelegte JSON-Snapshots pro Stadt (Unterordner, nicht Teil dieser Doku) |
| `__pycache__/` | Ordner | Python-Bytecode-Cache (generiert) |
...
flowchart TD
    ...
    CD[collect_data.py<br/><i>Orchestrator / CLI</i>] -->|liest| cities
    ...
    WS[web_server.py<br/><i>Flask API</i>] -->|importiert| DBU
    WS -->|liest| cities
    WS -->|SELECT| MariaDB
...
1. Lädt `cities.json`.
2. Erstellt für jede aktivierte Stadt via `COLLECTOR_MAP` die passende Collector-Instanz (`create_collector`).
...
Wird importiert von `base.py`, `collect_data.py` und `web_server.py`.
...
### `web_server.py`
Flask-App (Port 80) für die Anzeige der gesammelten Daten:
- Liefert statische Dateien (`index.html`, `logs.html`, `cities.json`, `config/*`).
- `/api/cities`, `/api/dates/<city>`, `/api/data/<city>/<date>` – Parkplatzdaten aus `pls_fetch_current`.
- `/api/logs`, `/api/logs/daily_counts` – Logs aus der Tabelle `log`.

Nutzt `db_utils.get_connection()` für alle DB-Zugriffe.

### Konfigurationsdateien
- **`cities.json`** – pro Stadt: `enabled`, `name`, `collector` (Klassenpfad, gemappt in `COLLECTOR_MAP`), `api_url`, `official_url`, `description`.
- **`.env`** – DB-Zugangsdaten, geladen via `python-dotenv` (`DB_HOST`, `DB_PORT`, `DB_USER`, `DB_PASSWORD`, `DB_NAME`).
- **`database-setup.sql`** – Schema/Stammdaten für `cities`, `parkhaeuser` u.a. Tabellen.
- **`requirements.txt`** – `requests`, `mysql-connector-python`, `schedule`, `flask`, `flask-cors` (Hinweis: `python-dotenv` und `pymysql`, die von `db_utils.py` bzw. `get_event_and_weather_data.py` importiert werden, fehlen hier aktuell in der Liste).
